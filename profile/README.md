...
## Outstake 
Outstake is a protocol built around native yield tokens, currently supporting the native yield features of Blast (ETH L2). It will later expand to multiple chains and multiple revenue tokens, while introducing a new type of non-USD stablecoin model tied to native yield rates. The protocol employs an innovative economic model that is more flexible and composable compared to other yield token protocols in the market, such as Pendle. It also offers higher yields and multiple revenue sources. The assets supported by Outstake form the foundational support for the Outrun ecosystem, providing a stable base for the entire ecosystem. Specifically, the working principles of Outstake are as follows:

+ **A new staking model based on native yield tokens**: Allows users to control the native revenue generated by their assets. They can mint Staked tokens to unlock the liquidity of their staked assets and use them across a range of DeFi applications.
